...
Chains crossing identity boundaries — for example, `DeclaredSession` → `OperationalSession` — carry the identity-tier consistency-across-types default per [`§0015`](./decision-log.md); inception-phase single-tier `actor_ref` adopted per [`§0023`](./decision-log.md) (Identity tiers — Open Modeling Question 1 resolution: single-tier at inception; multi-tier formalization deferred to ordinary Ontology RFC discipline). The single-tier commitment is binding today and structurally falsifiable; multi-tier formalization becomes structurally falsifiable when subsequently adopted via Ontology RFC.
...
- **§2.3 does not govern Identity-tier formal specifics.** The identity-tier consistency-across-types default per [`§0015`](./decision-log.md) carries through provenance chains today; inception-phase single-tier `actor_ref` adopted per [`§0023`](./decision-log.md) (Identity tiers — Open Modeling Question 1 resolution: single-tier at inception; multi-tier formalization deferred to ordinary Ontology RFC discipline). The single-tier commitment is binding today and structurally falsifiable; multi-tier formalization becomes structurally falsifiable when subsequently adopted via Ontology RFC.
...
- **§2.4 does not govern Identity-tier formal specifics.** The identity-tier consistency-across-types default per [`§0015`](./decision-log.md) carries through `influenced_by` chains today; inception-phase single-tier `actor_ref` adopted per [`§0023`](./decision-log.md) (Identity tiers — Open Modeling Question 1 resolution: single-tier at inception; multi-tier formalization deferred to ordinary Ontology RFC discipline). The single-tier commitment is binding today and structurally falsifiable; multi-tier formalization becomes structurally falsifiable when subsequently adopted via Ontology RFC.
...
The [§4 criterion 1](#4-constitutional-design-rule) discipline requires structural enforcement at the schemas layer, not runtime checks in consumer code. §2.6 codifies this enforcement obligation; the schemas implementation is per [`§0136`](./decision-log.md) canonical-serialization-contract. Together with [§2.4](#24-inferential-influence-disclosure)'s `influenced_by` chain commitment, §2.6 completes the two-dimensional structural defense the Thesis names: §2.4 records which chains an Assertion was formed under; §2.6 records how independent the Assertion's evidence is from the chain.

#### Forbidden Anti-Patterns

- **Confidence-only inferential-assertion records.** A Cat II construct or Cat III hypothesis record, or an Assertion with `subject_ref_construct` / `subject_ref_hypothesis` populated, committed with `confidence` present but `evidential_independence` absent. Detectable at canonical-serialization layer per [`§0136`](./decision-log.md); substrate-commit fails for records missing either dimension.
...
- **§2.6 governs the structural pairing of `confidence` + `evidential_independence`; not the formal measurable-quantity definition of independence.** The pairing's structural enforcement is binding today; the formal definition of `independence` as a measurable quantity is operational specification resolved per [`§0133`](./decision-log.md) (Q3 resolution — Candidate α: source-count ratio over Cat I provenance roots, under §2.6 BC2 meta-shape 1 deterministic-from-pattern). The pairing commitment is binding today and structurally falsifiable per §0133.
...
- **§2.6 does not govern Identity-tier formal specifics.** Inherited from [§2.4](#24-inferential-influence-disclosure) — identity-tier consistency-across-types default per [`§0015`](./decision-log.md) carries through chains; inception-phase single-tier `actor_ref` adopted per [`§0023`](./decision-log.md) (Identity tiers — Open Modeling Question 1 resolution: single-tier at inception; multi-tier formalization deferred to ordinary Ontology RFC discipline). The single-tier commitment is binding today and structurally falsifiable; multi-tier formalization becomes structurally falsifiable when subsequently adopted via Ontology RFC.

- **§2.6 does not govern the transitive scope of independence-along-chain semantics.** Inherited from [§2.4](#24-inferential-influence-disclosure) — propagation semantics codified in [`provenance-model.md` §Inferential Provenance](../ontology/provenance-model.md) carry through today; formal transitivity specification resolved per [`§0134`](./decision-log.md) (Q5 transitivity-half — Candidate τ: transitive closure of declared direct `influenced_by` edges, with β-graph storage). The Ontology-level commitment is binding today and structurally falsifiable per §0134.
...
**N1 — Ghost Trace does not produce truth.** Operationalized via [§2.4](#24-inferential-influence-disclosure) (`influenced_by` chain declaration) and [§2.6](#26-evidential-independence-integrity) (paired `confidence` + `evidential_independence` structural enforcement). A change set proposing a substrate-committed inferential record without declared influence chain or paired independence dimension is detectable at the canonical-serialization-contract layer per [`§0136`](./decision-log.md); rejection occurs at commit time.
...
- **Truth-bearing record committed to substrate.** A substrate-committed inferential record (Cat II construct, Cat III hypothesis, or Assertion with `subject_ref_construct` / `subject_ref_hypothesis`) marked with definitive-truth semantic without declared `influenced_by` chain per [§2.4](#24-inferential-influence-disclosure) and paired `evidential_independence` dimension per [§2.6](#26-evidential-independence-integrity). Detectable at the canonical-serialization-contract layer per [`§0136`](./decision-log.md); rejection occurs at commit time.
