...
- **§2.2 Epistemic Separation (frozen):** preserved. T1 commits Cat I records; T4 commits Cat III lifecycle records. No category boundary crosses at this RFC.
...
2. **Handler-side tier dispatch.** Each route is annotated with its tier (the [`§0094`](../../charter/decision-log.md) classification is the source). `ServeHTTP` looks up the route's tier; auth-check loads the per-tier token; constant-time compare against the request's `Authorization: Bearer <token>` value. 401 on mismatch with `WWW-Authenticate: Bearer realm="ghost-trace-ingestion"` per [`§0035`](../../charter/decision-log.md) convention. T0 (`/healthz`) remains exempt.
...
4. **Per-actor attribution on T3 + T4 IngestionEvent pairs.** For HTTP-channel T3 + T4 writes, the `IngestionEvent` paired with the substrate-mutating record carries the actor identity:
   - From the verified mTLS subject (CN + SAN + cert-SHA-256), when γ is active.
   - From the matched bearer-token's `token_id` (a new optional field in the per-tier token file: `<token>\n<token_id>\n`), when only α is active.
   - From a fallback `unattributed-token-<tier>` literal when only α is active AND no `token_id` is supplied in the token file — this is operationally discouraged (logged as a warning at startup) but not forbidden, to preserve the [`§0035`](../../charter/decision-log.md) single-line-token-file backward compatibility.
...
- **AP2 — Per-actor attribution dropped on retry.** A T3/T4 request retried without the `IngestionEvent`-pair commit succeeding leaves a substrate write unattributed. Mitigation: T3/T4 endpoints commit the lifecycle event AND its IngestionEvent in a single substrate transaction ([`§0038`](../../charter/decision-log.md) `AppendPair` is the existing precedent); partial failure rolls both back.
