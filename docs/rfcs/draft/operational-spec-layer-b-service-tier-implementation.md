...
This RFC opens structured discussion. Candidate decisions are enumerated in [`layer-b-service-tier-implementation-evidence.md`](../discussion/layer-b-service-tier-implementation-evidence.md) Phase 2 (six sub-decisions). Phases 3-5 of the evidence file produce the discussion-phase recommendation: **A1 (internal package) + B1 (on-the-fly) + C1 (substrate-global; structurally determined by canonical-serialization-contract) + D1 (transient DemoteReport extension) + E1 (advisory like Layer A) + F3 (CLI operator-supplied with bundled defaults)** — conservative-defaults bundle aligned with the [`§0011`](../../charter/decision-log.md) Layer A pattern precedent and the [`§0022`](../../charter/decision-log.md) + [`§0023`](../../charter/decision-log.md) inception-phase posture.
...
- If E1 (advisory-like-Layer-A) is adopted: existing CLIs gain Layer B state in DemoteReport, same shape as the existing CadenceSatisfied field. No commit semantics change; the report surface expands.
