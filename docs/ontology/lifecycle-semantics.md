...
- **Question 4 (Independence-driven lifecycle, originally a narrower binary framing of canonical Ontology Q4).** Resolved by [`decision-log.md` §0011 — Q4 resolution](../charter/decision-log.md) as the staged-combination form (Layer A AND Layer B). The previously framed binary (automatic-on-threshold vs operator-driven) is superseded: the resolution is neither alternative as framed, but a staged structural form. Layer B's inner structure is further resolved by [`decision-log.md` §0135 — Layer B resolution](../charter/decision-log.md) as L-BC-OR (disjunctive — `(freshness_B < T_B) OR (saturation_C > K_C)`) with the L-C structural-exclusion commitment. Inception-phase parameter values are resolved by [`decision-log.md` §0138 — Layer B parameter-calibration resolution](../charter/decision-log.md): `T_B = K_C = 0.5`; `N = 1000`; window form W-count; per-subtype divergence U-uniform; `N_A = 1 day` (bundled). Service-tier implementation strategy is resolved by [`decision-log.md` §0141 — Layer B service-tier implementation resolution](../charter/decision-log.md): conservative-defaults bundle A1 (internal package) + B1 (on-the-fly) + C1 (substrate-global, structurally determined by canonical-serialization-contract) + D1 (transient DemoteReport) + E1 (advisory like Layer A) + F3 (CLI operator-supplied with bundled defaults). The full Q4 → Layer B operational arc (§0011 → §0099 → §0129 → §0133 → §0134 → §0135 → §0136 → §0138 → §0141) is structurally complete at all three tiers (form, parameters, implementation).

<!-- Invariant 2.5 (Hypothesis Lifecycle Explicitness) frozen v0.3 per `decision-log.md` §0013. The Category III section + §The Promotion Mechanism + §Cross-subtype operations above carry the six lifecycle operations as explicit transition events with structural commitments at formation, merge, split, promotion, demotion, and dissolution; per §2.5 BC5, each is a Category I immutable record about a Category III entity. -->

<!-- Coordination with [`provenance-model.md`](./provenance-model.md) on lifecycle event appearance in the provenance graph is discharged at [`provenance-model.md` §Inferential Provenance §Decay via §2.5 lifecycle event supersession](./provenance-model.md): the substrate-committed `influenced_by` edge is immutable per §2.1; current operational state is a derived Category II projection over the §2.5 lifecycle event chain of the referenced hypothesis. -->
