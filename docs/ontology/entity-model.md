...
- **Authentication-class typing on Cat I observation envelopes** (F4 anticipated at [`decision-log §0143`](../charter/decision-log.md) Domain Pack v0.1 anticipated-OMQs table; trigger fired at [`decision-log §0147`](../charter/decision-log.md); opened at [`decision-log §0148`](../charter/decision-log.md); substantive deliberation at [`decision-log §0149`](../charter/decision-log.md)). Whether F1 Cat I observation envelopes should carry a structural surface distinguishing how the observed actor's identity was verified at observation time (server-authenticated / client-attested / client-witnessed). **Resolved** by [`decision-log.md` §0150 — Candidate α resolution](../charter/decision-log.md): envelope `authentication_class` proto3 enum field added to each F1 envelope (NetworkObservation + BehavioralObservation + AttestationObservation + BrowserObservation-when-it-lands); 4 enum values (UNKNOWN / SERVER_AUTHENTICATED / CLIENT_ATTESTED / CLIENT_WITNESSED) defined in [`common/v1/authentication_class.proto`](../../schemas/common/v1/authentication_class.proto); default UNKNOWN preserves binary backward-compat for pre-resolution records. Reversal conditions documented per [`§0023`](../charter/decision-log.md) + [`§0141`](../charter/decision-log.md) pattern.
