...
- **Propagation rules.** Influence accumulates transitively through chains of derivation: when Assertion A is `influenced_by` Hypothesis H₁, and H₁ was formed using observations and prior hypotheses, A's inferential provenance closure includes H₁'s antecedents to the extent they affected H₁'s formation. Transitive scope resolved per [`§0134`](../charter/decision-log.md) (Q5 transitivity-half — Candidate τ): per-record closure is the transitive closure of declared direct `influenced_by` edges, with β-graph storage (substrate stores direct edges + per-record cached closures committed at formation time per [`§0021`](../charter/decision-log.md) OMQ #3 — Candidate α).
...
- **Relationship to evidential independence (Invariant 2.6 frozen v0.6 per [`§0129`](../charter/decision-log.md)).** Inferential provenance, post-supersession-filtering, is the structural input to independence computation. The evidence-staleness or influence-saturation computation per Layer B's deep criterion (per [`ontology-revision-layer-b-deep-criterion`](../rfcs/draft/ontology-revision-layer-b-deep-criterion.md), resolved at [`§0135`](../charter/decision-log.md) as L-BC-OR with the L-C structural-exclusion commitment; inception-phase parameters fixed at [`§0138`](../charter/decision-log.md)) consumes the superseded-state-annotated graph directly per §2.5 BC5.
...
<!-- Invariant 2.4 (Inferential Influence Disclosure) frozen v0.5 per `decision-log.md` §0099. The §Inferential Provenance section above carries the structural requirement language (typed `influenced_by` edges committed at substrate event time per §0021 — Candidate α; transitive scope per §0134 — Candidate τ; decay via §2.5 lifecycle event supersession per §0020 — Candidate C); §2.4 binding text anchors it constitutionally. -->

<!-- Coordination with [`lifecycle-semantics.md`](./lifecycle-semantics.md) on lifecycle event propagation through the provenance graph is discharged at §Inferential Provenance §Decay via §2.5 lifecycle event supersession above (per §2.5 BC5 — lifecycle events are Category I records about Category III entities). -->
