...
Both fields are declared in the proto definition of every record subject to the commitment. Enforcement of `evidential_independence` presence at substrate commit is performed by an explicit canonical-package check (`validatePairedDimensionCommitment` in the canonical-serialization library) installed per [`§0140`](../charter/decision-log.md). The check is asymmetric:

- **`evidential_independence`**: REQUIRED to be present at commit time. The field is a proto3 message with explicit field-presence (`m.Has(fd)`); absence is a structural commitment violation under [Charter §2.6](../charter/constitutional-charter.md#26-evidential-independence-integrity) v0.6 anti-pattern 1 (paired-dimension bypass) and is rejected at the marshalling boundary by `validatePairedDimensionCommitment`.

- **`confidence`**: NOT enforced for presence at the marshalling boundary. The field is a proto3 scalar (`float`) with no field-presence (default-value `0.0` is structurally indistinguishable from "not set"), and `confidence = 0.0` is a valid low-confidence commitment under the confidence semantic. Presence enforcement for `confidence` is therefore not the marshalling-boundary's responsibility.

The `AllowPartial: false` option in the canonical Marshal call (per the §Marshal call section above) is a defense-in-depth boundary for proto2-derived types should any be introduced; for proto3 — the schemas-technology selection per [`§0024`](../charter/decision-log.md) — `AllowPartial: false` is structurally a no-op (proto3 has no `required` fields). [`§0140`](../charter/decision-log.md) records the patch-via-pressure history that surfaced this clarification.

Confidence and evidential_independence are structurally independent at substrate per [Charter §2.6](../charter/constitutional-charter.md#26-evidential-independence-integrity) — neither is derivable from the other at commit time.

### Validation discipline

Substrate-commit fails at the canonical-marshalling boundary when a record subject to the commitment has `evidential_independence` absent at commit time (per the asymmetric enforcement above). The failure mode is deterministic and not bypassable at the consumer layer. The check is structural-only — it does NOT verify the rational-pair validity (denominator > 0; numerator ≤ denominator), which is performed by the supplementary `validateEvidentialIndependence` walker per the [§Evidential Independence](#evidential-independence-q3-α) section below.
...
- **Paired-dimension bypass.** Committing a record subject to the paired-dimension commitment (records declaring BOTH a `confidence` field AND an `evidential_independence` field per the [§Paired-Dimension Commitment](#paired-dimension-commitment) section above) with `evidential_independence` absent at commit time. Forbidden by [§2.6](../charter/constitutional-charter.md#26-evidential-independence-integrity) v0.6 anti-pattern 1. Detectable at the canonical-marshalling boundary via the asymmetric explicit field-presence check installed per [`§0140`](../charter/decision-log.md) (`validatePairedDimensionCommitment`); the `AllowPartial: false` option in the Marshal call provides defense-in-depth for proto2-derived types should any be introduced but is structurally a no-op for proto3 per the §Marshal call section above. Confidence presence is not part of this anti-pattern — proto3 scalar fields have no field-presence and `confidence = 0.0` is a valid low-confidence commitment.
...
- [`docs/charter/decision-log.md` §0140](../charter/decision-log.md) — Paired-dimension marshalling-boundary enforcement; the §Paired-Dimension Commitment §Required fields paragraph's `AllowPartial: false` claim corrected (structurally a no-op for proto3) + an explicit canonical-package check (`validatePairedDimensionCommitment`) installed at the marshalling boundary enforcing `evidential_independence` presence on records subject to the commitment. Asymmetric enforcement by design: confidence presence is not enforced (proto3 scalar; 0.0 is valid). This contract's Paired-Dimension Commitment + Anti-Patterns sections revised.
