...
**Structural choice (per [`§0049`](../../docs/charter/decision-log.md)).** The merge event references both antecedent formation hashes AND a separately-committed produced formation hash — rather than collapsing the produced hypothesis identity into the merge event itself. This preserves the [`§0045`](../../docs/charter/decision-log.md) invariant ("hypothesis identity IS the formation event's content-hash") so every lifecycle operation (form, promote, demote, dissolve, merge, split) targets hypotheses through formation hashes uniformly. The produced formation must be created separately (typically via `form-hypothesis` against a substrate populated with the union of source observations) before the merge is recorded.
...
**Per-actor attribution.** The paired `IngestionEvent` carries the channel (`http`, `https`, or `https+mtls`) and the verified client identity (mTLS CN + SAN + cert SHA-256) when mTLS is configured. Bearer-token-only deployments record the channel + (per the auth-scope RFC item 4(b)) the matched per-tier token's `token_id` when the substrate-admin token file's optional second line is configured (`<token>\n<token_id>\n`); legacy single-line shape preserves backward compatibility per RFC item 4(c). The substrate-admin tier token authorizes T3 writes; the audit record's existence + the paired `IngestionEvent`'s client identity are the substrate-time evidentiary record.

**Distinction from CLI orphan-cleanup.** [`cmd/orphan-cleanup`](#orphan-cleanup-cli) defaults to NO substrate audit record — the CLI runs under the §0033 local-shell-trust assumption. Per decision-log §0119 (auth-scope RFC Open Question 4 discharge), the CLI supports an opt-in `--actor` option that activates the same audit-on-commit discipline as the HTTP T3 path: when `--actor=<id>` is non-empty, the CLI commits an `OrphanCleanupAudit` paired with an `IngestionEvent` (channel=`cli`, client_common_name=`<id>`) BEFORE deletion, mirroring the HTTP audit-then-delete contract. When `--actor` is empty the §0033 no-audit behavior is preserved (backward-compatible).

### HTTP T4 constitutional-act endpoints (promote across all four subtypes)

Per [`§0105`](../../docs/charter/decision-log.md) (pilot landing) + [`§0106`](../../docs/charter/decision-log.md) (replication across remaining subtypes) + the auth-scope RFC at [`§0098`](../../docs/charter/decision-log.md), the HTTP interface ships the promote endpoint across all four Cat III subtypes:

- `POST /v1/hypotheses/behavioral-cluster/promote`
- `POST /v1/hypotheses/automation-group/promote`
...
**Per-actor attribution.** The paired `IngestionEvent` carries the channel (`http`, `https`, or `https+mtls`) and the verified client identity (mTLS CN + SAN + cert SHA-256) when mTLS is configured. For bearer-token-only deployments, per-token attribution surfaces the matched constitutional-act token's `token_id` per the auth-scope RFC item 4(b) when the per-tier token file's optional second line is configured (`<token>\n<token_id>\n`); when only the legacy single-line shape is configured the per-actor field falls back to the literal `unattributed-token-constitutional-act` per RFC item 4(c). The single-line fallback is operationally discouraged but preserved for §0035 backward compatibility.

**Resolver precedence (T4).** `resolveT4Actor` returns the first non-empty value in: (1) verified mTLS subject CN (γ active); (2) matched per-tier token's `token_id` (α active, second-line configured per RFC item 4(b)); (3) `unattributed-token-<tier>` literal (α active, single-line legacy shape). The middle rung is materialized by a `WithAuthTierTokenID` option per-tier; the option is operator-supplied by reading the per-tier token file's second line at service startup.
...
**Cross-subtype symmetry.** The four promote endpoints share the same wire shape (`application/x-protobuf` request → JSON response), the same per-actor attribution precedence (mTLS subject CN → `token_id` → fallback literal), the same validation gates (32-byte formation hash + positive `cadence_seconds`), and the same error semantics (404 on unknown formation OR wrong-type formation hash; the wrong-type check rejects cross-subtype formation hashes — e.g., an AutomationGroup formation hash on the BehavioralCluster endpoint returns 404 with `wrong type` semantics per §2.5 BC5). Each handler delegates to the subtype-specific `hypothesis.Promote*` helper, which extends the §0097 BehavioralCluster pilot's `Actor` + `IngestionEvent` pairing pattern to all four subtypes.
