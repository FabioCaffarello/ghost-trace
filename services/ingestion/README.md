...
**Subtype-aware** (per [`§0062`](../../docs/charter/decision-log.md) + [`§0069`](../../docs/charter/decision-log.md)): auto-detects the formation's Cat III subtype (`behavioral_cluster`, `automation_group`, or `campaign_hypothesis`) by looking up the formation row's message_type, then dispatches to the appropriate per-subtype projection. The JSON output includes a `subtype` field.
...
**Subtype-aware** (per [`§0062`](../../docs/charter/decision-log.md) + [`§0069`](../../docs/charter/decision-log.md)): returns projections for ALL THREE `behavioral_cluster`, `automation_group`, and `campaign_hypothesis` subtypes by default. The `-subtype` option filters to one. Each entry includes a `subtype` field.
...
| Option | Default | Notes |
|---|---|---|
| `-subtype` | empty | Filter by Cat III subtype: `behavioral_cluster`, `automation_group`, or `campaign_hypothesis`. Empty = all subtypes. |
...
Output is structured JSON with **per-subtype sections** (per [`§0062`](../../docs/charter/decision-log.md) + [`§0069`](../../docs/charter/decision-log.md)): `behavioral_cluster`, `automation_group`, and `campaign_hypothesis` each carry `total` (formation count) + `by_state` (map from state value to count). Every State key is present (even at zero) — predictable wire shape.
