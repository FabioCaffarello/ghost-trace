...
func TestCombineCountsEmpty(t *testing.T) {
	...
	wantStates := []State{
		StateForming, StatePromoted, StateDemoted,
		StateDissolved, StateMergedInto, StateSplitInto,
	}
...
func TestCombineCountsSumsTotalsAndPerStateBuckets(t *testing.T) {
	bc := StateCounts{
		...
		ByState: map[State]int{
			StateForming: 3, StatePromoted: 1, StateDemoted: 0,
			StateDissolved: 1, StateMergedInto: 0, StateSplitInto: 0,
		},
	}
	ag := StateCounts{
		Total: 4,
		ByState: map[State]int{
			StateForming: 1, StatePromoted: 2, StateDemoted: 1,
			StateDissolved: 0, StateMergedInto: 0, StateSplitInto: 0,
		},
	}
	ch := StateCounts{
		Total: 2,
		ByState: map[State]int{
			StateForming: 0, StatePromoted: 0, StateDemoted: 0,
			StateDissolved: 0, StateMergedInto: 1, StateSplitInto: 1,
		},
	}
	...
	wantByState := map[State]int{
		StateForming: 4, StatePromoted: 3, StateDemoted: 1,
		StateDissolved: 1, StateMergedInto: 1, StateSplitInto: 1,
	}
...
func TestCombineCountsPopulatesAllStateKeys(t *testing.T) {
	partial := StateCounts{
		Total:   1,
		ByState: map[State]int{StateForming: 1},
	}
...
func TestAggregateLatenciesNearestRank(t *testing.T) {
	...
	got := AggregateLatencies([]int64{30, 10, 50, 40, 20})
...
func TestCombineLatencySamplesUnionsAcrossSubtypes(t *testing.T) {
	bc := LatencySamples{
	...
	combined := CombineLatencySamples(bc, ag)
...
func TestCombinedPercentilesAreExactNotApproximated(t *testing.T) {
	...
	combined := append([]int64(nil), bcSamples...)
	combined = append(combined, agSamples...)
	// Union [10,20,30,40,50,60,70]; n=7; p50 index = ceil(7*0.5)-1 = 4-1 = 3 → 40
	got := AggregateLatencies(combined)
	if *got.P50Ns != 40 {
