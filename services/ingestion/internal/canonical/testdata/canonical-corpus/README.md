...
| `campaign-hypothesis-formation-minimal` | all proto3 defaults (`{}`) | First entry covering the THIRD Cat III concrete subtype (per [`§0063`](../../../../../../docs/charter/decision-log.md)). First event-centric subtype (no `actor_refs` field). |
| `campaign-hypothesis-formation-typical` | typical temporal-descriptor-cohort-v1 formation | Exercises all five `CampaignHypothesisFormation` fields: `pattern_signature` = `temporal-descriptor-cohort-v1`, `pattern_parameters` with both keys, `formation_at`, placeholder `confidence`, three sorted `source_event_hashes` serving DUAL purpose (membership + provenance) per §0063. |
| `campaign-hypothesis-promotion-minimal` | all proto3 defaults (`{}`) | First entry covering the third-subtype promotion lifecycle event (§0064). |
| `campaign-hypothesis-promotion-typical` | typical operator-invoked promotion values | Exercises all four `CampaignHypothesisPromotion` fields: `formation_event_hash` (synthetic 32-byte pattern), `promoted_at`, Layer A `cadence_seconds`, free-form `reason`. |
| `campaign-hypothesis-demotion-minimal` | all proto3 defaults (`{}`) | First entry covering the third-subtype demotion lifecycle event (§0065). |
| `campaign-hypothesis-demotion-typical` | typical operator-invoked demotion values | Exercises all three `CampaignHypothesisDemotion` fields: `promotion_event_hash` (synthetic 32-byte pattern), `demoted_at`, free-form `reason`. |
| `campaign-hypothesis-dissolution-minimal` | all proto3 defaults (`{}`) | First entry covering the third-subtype dissolution lifecycle event (§0066). |
| `campaign-hypothesis-dissolution-typical` | typical operator-invoked dissolution values | Exercises all three `CampaignHypothesisDissolution` fields: `formation_event_hash` (synthetic 32-byte pattern), `dissolved_at`, free-form `reason`. |
| `campaign-hypothesis-merge-minimal` | all proto3 defaults (`{}`) | First entry covering the third-subtype merge lifecycle event (§0067). |
| `campaign-hypothesis-merge-typical` | typical within-subtype merge values | Exercises all four `CampaignHypothesisMerge` fields: two ascending-sorted antecedent hashes, produced formation hash, `merged_at`, free-form `reason`. |
