...
type AutomationGroupDissolveOptions struct {
	...
	// Actor per §0108 — non-empty triggers AppendPair with IngestionEvent.
	Actor string
}
...
func DissolveAutomationGroup(ctx context.Context, sub *substrate.Substrate, opts AutomationGroupDissolveOptions, now func() time.Time) (AutomationGroupDissolveReport, error) {
	...
	if err != nil {
		return AutomationGroupDissolveReport{}, fmt.Errorf("hypothesis.DissolveAutomationGroup: marshal ingestion event: %w", err)
	}
	...
	if err := sub.AppendPair(ctx, dissRow, payload, ingRow, ingPayload); err != nil {
		return AutomationGroupDissolveReport{}, fmt.Errorf("hypothesis.DissolveAutomationGroup: append pair (dissolution %s, ingestion %s): %w", hex, ingHex, err)
	}
