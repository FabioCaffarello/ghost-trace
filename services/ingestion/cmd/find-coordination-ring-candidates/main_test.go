...
// TestFullPipeline_EndToEnd verifies CoordinationRing subtype emission
// in the JSON envelope's hypothesis_subtype field + Interactions field
// populated per §0185 interaction-centric ontology.
func TestFullPipeline_EndToEnd(t *testing.T) {
	...
	tmp, err := os.CreateTemp("", "find-coordination-ring-candidates-*.json")
	if err != nil {
	...
	if got.SignatureName != "endpoint_co_visit_v1" {
	...
	if c.HypothesisSubtype != "CoordinationRing" {
		t.Errorf("HypothesisSubtype: got %q want CoordinationRing (first F3 emission of this subtype per §0185)", c.HypothesisSubtype)
	}
	// 3 actors → 3 edges: (a,b), (a,c), (b,c).
	if len(c.Interactions) != 3 {
		t.Errorf("Interactions count: got %d want 3 (3-actor ring → 3 edges)", len(c.Interactions))
	}
	// Per-§0070 within-edge lex: every edge MUST have edge[0] < edge[1].
...
// candidate case emits a valid JSON envelope per §0154 MO4 + §0163 MO3
// 4-test contract.
func TestFullPipeline_BelowThreshold_NoCandidates(t *testing.T) {
	...
	tmp, err := os.CreateTemp("", "find-coordination-ring-candidates-empty-*.json")
	if err != nil {
		t.Fatalf("CreateTemp: %v", err)
	}
	defer os.Remove(tmp.Name())
