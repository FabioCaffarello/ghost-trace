...
func run() error {
	...
	formationHex := flag.String("formation-event-hash", "", "REQUIRED: hex-encoded BLAKE3-256 content-hash of a Cat III formation event (BehavioralCluster, AutomationGroup, CampaignHypothesis, or CoordinationRing — subtype auto-detected)")
	flag.Parse()
	...
	case "ghosttrace.events.v1.CoordinationRingFormation":
		proj, err := projection.ProjectCoordinationRing(ctx, sub, formationHash)
		if err != nil {
			return err
		}
		...
		fmt.Fprintf(os.Stderr,
			"hypothesis-state: subtype=coordination_ring formation=%s state=%s lifecycle_events=%d\n",
			*formationHex, proj.State, len(proj.LifecycleHistory))
...
func buildCROutput(formationHex string, p projection.CoordinationRingProjection) output {
	o := output{
