// Command demote-coordination-ring records a CoordinationRing
// demotion lifecycle event per Charter §2.5 + decision-log §0072 —
// third lifecycle operation of the fourth Cat III subtype arc
// (mirrors §0047 BC, §0058 AG, §0065 CH).
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "demote-coordination-ring: %v\n", err)
...
func run() error {
	...
	promotionHashHex := flag.String("promotion-event-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the target CoordinationRingPromotion")
	...
	report, err := hypothesis.DemoteCoordinationRing(ctx, sub, hypothesis.CoordinationRingDemoteOptions{
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"demote-coordination-ring: promotion=%s demotion=%s ingestion=%s cadence_satisfied=%v elapsed=%ds %s actor=%q\n",
		...
		fmt.Fprintf(os.Stderr,
			"demote-coordination-ring: promotion=%s demotion=%s cadence_satisfied=%v elapsed=%ds %s\n",
