// Command replay-automation-group-formation performs Phase 3
// reconstructive replay of an AutomationGroupFormation per
// decision-log §0087. Mirrors §0086 (BC Phase 3 replay) shape;
// AutomationGroup-specific pattern resolver.
//
// Exit codes: 0 success AND match=true; 1 success AND match=false
// (drift); 2 tool/config error; 3 substrate-precondition failure.
package main
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "replay-automation-group-formation: %v\n", err)
...
func run() error {
	...
	targetHashHex := flag.String("target-event-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the AutomationGroupFormation to replay")
	flag.Parse()
	...
	report, err := replay.ReplayAutomationGroupFormation(ctx, sub, targetHash)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(os.Stderr,
		"replay-automation-group-formation: target=%s match=%v reconstructed=%d contributors=%d max_committed_at=%d\n",
