// Command dissolve-automation-group records an AutomationGroup
// dissolution lifecycle event per Charter §2.5 + decision-log §0059
// — fourth lifecycle operation of the second Cat III subtype arc
// (mirrors §0048 for the BehavioralCluster arc).
//
...
// The distinction transfers across subtypes.
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "dissolve-automation-group: %v\n", err)
...
func run() error {
	...
	report, err := hypothesis.DissolveAutomationGroup(ctx, sub, hypothesis.AutomationGroupDissolveOptions{
