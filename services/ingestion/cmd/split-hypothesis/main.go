// Command split-hypothesis records a Category III hypothesis split
// lifecycle event per Charter §2.5 + lifecycle-semantics.md line 29
// — sixth (and final) of six Cat III lifecycle operations. Wraps
// internal/hypothesis.Split with command-line configuration +
// structured output.
//
// SCOPE — within-subtype only at this layer. The antecedent and all
// successor formation hashes MUST resolve to BehavioralClusterFormation
// rows. Cross-subtype split per entity-model.md §Cross-subtype
// operations remains deferred to lifecycle-semantics.md post-Q4
// redaction.
//
// Split is the STRUCTURAL INVERSE of merge per §0049: merge is 2-to-1
// (two antecedents, one produced), split is 1-to-N (one antecedent,
// multiple successors). Each successor is a separately-committed
// BehavioralClusterFormation, mirroring §0049 Option B; this
// preserves the §0045 invariant (hypothesis identity = formation
// content-hash) across all six lifecycle operations.
//
// Successors form a SET per lifecycle-semantics.md line 29 — the
// CLI sorts the successor hashes ascending before recording so the
// split event's content-hash is invariant under the order the
// operator enumerates the -successor-formation-hash flags.
//
...
// target-wrong-type OR insufficient-successors OR successors-not-
// distinct (the §2.5-lifecycle-integrity errors).
package main
...
// successorList is a repeated flag accumulator. Each -successor-
// formation-hash invocation appends one hex-encoded hash.
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "split-hypothesis: %v\n", err)
...
func run() error {
	...
	antecedentHex := flag.String("antecedent-formation-hash", "", "REQUIRED: hex-encoded BLAKE3-256 content-hash of the BehavioralClusterFormation being split")
	var successors successorList
	flag.Var(&successors, "successor-formation-hash", "REQUIRED (≥ 2 invocations): hex-encoded BLAKE3-256 content-hash of a successor BehavioralClusterFormation; repeat the flag for each successor")
	splitAtNs := flag.Int64("split-at-ns", 0, "explicit split_at as Unix nanoseconds; 0 = wall-clock now()")
	reason := flag.String("reason", "", "operator-supplied forensic note explaining the recognition that the antecedent hypothesis contained multiple distinct phenomena; strongly recommended")
	...
	report, err := hypothesis.Split(ctx, sub, hypothesis.SplitOptions{
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"split-hypothesis: antecedent=%s successors=%d split=%s ingestion=%s actor=%q already_split=%v\n",
			*antecedentHex, len(successors.hexes), report.SplitEventHashHex, report.IngestionEventHashHex, *actor, report.AlreadySplit)
	} else {
		fmt.Fprintf(os.Stderr,
			"split-hypothesis: antecedent=%s successors=%d split=%s already_split=%v\n",
