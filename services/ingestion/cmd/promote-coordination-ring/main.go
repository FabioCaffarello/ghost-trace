// Command promote-coordination-ring records a CoordinationRing
// promotion lifecycle event per Charter §2.5 + decision-log §0071 —
// second lifecycle operation of the fourth Cat III subtype arc
// (mirrors §0046 BC, §0057 AG, §0064 CH).
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "promote-coordination-ring: %v\n", err)
...
func run() error {
	...
	report, err := hypothesis.PromoteCoordinationRing(ctx, sub, hypothesis.CoordinationRingPromoteOptions{
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"promote-coordination-ring: formation=%s promotion=%s ingestion=%s cadence_seconds=%d %s actor=%q already_promoted=%v\n",
		...
		fmt.Fprintf(os.Stderr,
			"promote-coordination-ring: formation=%s promotion=%s cadence_seconds=%d %s already_promoted=%v\n",
