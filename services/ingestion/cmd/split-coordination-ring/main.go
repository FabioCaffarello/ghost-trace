// Command split-coordination-ring records a CoordinationRing
// split lifecycle event per Charter §2.5 + lifecycle-semantics.md
// line 29 + decision-log §0075 — SIXTH (final) lifecycle operation
// of the fourth Cat III subtype arc (mirrors §0050 BC, §0061 AG,
// §0068 CH). Completes the §2.5 lifecycle surface across all four
// Cat III subtypes.
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "split-coordination-ring: %v\n", err)
...
func run() error {
	...
	antecedentHex := flag.String("antecedent-formation-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the CoordinationRingFormation being split")
	var successors successorList
	flag.Var(&successors, "successor-formation-hash", "REQUIRED (≥ 2 invocations): hex-encoded BLAKE3-256 of a successor CoordinationRingFormation; repeat the option for each successor")
	...
	report, err := hypothesis.SplitCoordinationRing(ctx, sub, hypothesis.CoordinationRingSplitOptions{
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"split-coordination-ring: antecedent=%s successors=%d split=%s ingestion=%s actor=%q already_split=%v\n",
			*antecedentHex, len(successors.hexes), report.SplitEventHashHex, report.IngestionEventHashHex, *actor, report.AlreadySplit)
	} else {
		fmt.Fprintf(os.Stderr,
			"split-coordination-ring: antecedent=%s successors=%d split=%s already_split=%v\n",
