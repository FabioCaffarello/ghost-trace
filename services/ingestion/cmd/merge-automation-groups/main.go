// Command merge-automation-groups records an AutomationGroup merge
// lifecycle event per Charter §2.5 + lifecycle-semantics.md line 28
// + decision-log §0060 — fifth lifecycle operation of the second
// Cat III subtype arc (mirrors §0049 for the BehavioralCluster arc).
//
// SCOPE — within-subtype only. All three formation hashes MUST
// resolve to AutomationGroupFormation rows.
//
// Merge is SYMMETRIC: ascending-sort of antecedent hashes before
// recording ensures invocations with swapped flags produce the same
// merge event content-hash.
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "merge-automation-groups: %v\n", err)
...
func run() error {
	...
	antecedentAHex := flag.String("antecedent-a-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the FIRST AutomationGroupFormation")
	antecedentBHex := flag.String("antecedent-b-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the SECOND AutomationGroupFormation")
	...
	report, err := hypothesis.MergeAutomationGroup(ctx, sub, hypothesis.AutomationGroupMergeOptions{
