...
func run() error {
	...
	layerB := flag.Bool("layer-b", false, "OPTIONAL per decision-log §0141 F3: when true, populate the promotion event's layer_b_parameters with §0138 inception-phase resolved values (T_B=K_C=1/2; N=1000; N_A from --cadence-seconds). Enables Layer B evaluation at subsequent demote per §0141 E1. Default false preserves the legacy path.")
	flag.Parse()
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"promote-hypothesis: formation=%s promotion=%s ingestion=%s cadence_seconds=%d %s actor=%q already_promoted=%v\n",
		...
		fmt.Fprintf(os.Stderr,
			"promote-hypothesis: formation=%s promotion=%s cadence_seconds=%d %s already_promoted=%v\n",
