// Command split-automation-group records an AutomationGroup split
// lifecycle event per Charter §2.5 + lifecycle-semantics.md line 29
// + decision-log §0061 — SIXTH (final) lifecycle operation of the
// second Cat III subtype arc (mirrors §0050).
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "split-automation-group: %v\n", err)
...
func run() error {
	...
	antecedentHex := flag.String("antecedent-formation-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the AutomationGroupFormation being split")
	var successors successorList
	flag.Var(&successors, "successor-formation-hash", "REQUIRED (≥ 2 invocations): hex-encoded BLAKE3-256 of a successor AutomationGroupFormation; repeat the option for each successor")
	...
	report, err := hypothesis.SplitAutomationGroup(ctx, sub, hypothesis.AutomationGroupSplitOptions{
	...
	if *actor != "" {
		fmt.Fprintf(os.Stderr,
			"split-automation-group: antecedent=%s successors=%d split=%s ingestion=%s actor=%q already_split=%v\n",
			*antecedentHex, len(successors.hexes), report.SplitEventHashHex, report.IngestionEventHashHex, *actor, report.AlreadySplit)
	} else {
		fmt.Fprintf(os.Stderr,
			"split-automation-group: antecedent=%s successors=%d split=%s already_split=%v\n",
