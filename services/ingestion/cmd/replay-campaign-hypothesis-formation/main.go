// Command replay-campaign-hypothesis-formation performs Phase 3
// reconstructive replay of a CampaignHypothesisFormation per
// decision-log §0088. Mirrors §0086 (BC) + §0087 (AG); third of
// four subtype-specific Phase 3 replay tools.
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "replay-campaign-hypothesis-formation: %v\n", err)
...
func run() error {
	...
	targetHashHex := flag.String("target-event-hash", "", "REQUIRED: hex-encoded BLAKE3-256 of the CampaignHypothesisFormation to replay")
	flag.Parse()
	...
	report, err := replay.ReplayCampaignHypothesisFormation(ctx, sub, targetHash)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(os.Stderr,
		"replay-campaign-hypothesis-formation: target=%s match=%v reconstructed=%d contributors=%d max_committed_at=%d\n",
