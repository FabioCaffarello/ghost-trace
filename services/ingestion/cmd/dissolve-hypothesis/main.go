// Command dissolve-hypothesis records a Category III hypothesis
// dissolution lifecycle event per Charter §2.5 + lifecycle-semantics.md
// §Hypothesis (Category III) line 36 — fourth Cat III lifecycle
// operation. Wraps internal/hypothesis.Dissolve with command-line
// configuration + structured output.
//
// Dissolution is DISTINGUISHED from demotion per glossary: demotion
// withdraws OPERATIONAL USE; dissolution recognizes NON-EXISTENCE of
// the underlying phenomenon. The two operations target different
// referents — demote-hypothesis targets a specific promotion event;
// dissolve-hypothesis targets the formation (hypothesis identity).
// Dissolution may be invoked regardless of whether the hypothesis was
// ever promoted.
//
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "dissolve-hypothesis: %v\n", err)
...
func run() error {
	...
	report, err := hypothesis.Dissolve(ctx, sub, hypothesis.DissolveOptions{
