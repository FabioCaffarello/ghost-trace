...
| **4.3** | Fix what the curve found — and after 4.4 the decision is a **durability trade, not an optimisation**. The archive does not ack until the commit succeeds, so a crash before the ack is already handled by redelivery; the two fsyncs protect exactly one case, a crash AFTER the ack. Weakening them means acked records can be lost to power loss while the stream still holds them for its retention window. That is an ADR. Third options exist and should be weighed first: fsyncing the shard directory once per batch, or not writing a blob at all for payloads small enough to live in the row. Deliberately unspecified here — naming the fix before taking the measurement is how a phase ends up with a well-tested fix for the wrong thing. **The curve has now been taken, and it re-scopes this: the mutex is real (0.70x speedup on eight workers) and is NOT the constraint (it saturates ~2.9M batches/s against an HTTP path that bends at ~8k sessions/s). The constraint is the archive at 1 356 commits/s against a collector publishing ~16k records/s.** See [the curve](../docs/results/load-curve-2026-08-05.md). | M |
